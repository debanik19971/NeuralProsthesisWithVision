...
model2.add(Conv2D(32,(3,3),activation = 'relu', input_shape = (64,64,1), kernel_initializer=initializers.RandomNormal(stddev=0.001)))
model2.add(MaxPooling2D(pool_size = (2,2),padding='same'))
model2.add(BatchNormalization())
#model2.add(Dropout(0.1))
model2.add(Conv2D(64, (3,3), activation='relu', kernel_initializer=initializers.RandomNormal(stddev=0.001)))
...
model2.add(Conv2D(256, (3,3), activation='relu', kernel_initializer=initializers.RandomNormal(stddev=0.001)))
...
model2.add(Flatten())
model2.add(Dense(1, activation = 'sigmoid',kernel_initializer=initializers.RandomNormal(stddev=0.001)))
model2.load_weights('objectnessnetworkiou_2objects_nearest_iou_in_twoobjects.05.h5')
	...
	while(True):
		if(stopstore==1 and stopcreation==1  and stoplaser==1):
			...
			for i in range(len(filteredboundingboxes)):
				# print(filteredobjectness[len(filteredboundingboxes)-i-1])
				...
				# rectangle=filteredboundingboxes[len(filteredboundingboxes)-i-1]
				# rectangle=boundingboxesset[-1][len(boundingboxesset[-1])-i-1]
				# rectangle=objectnessboundingboxes[len(objectnessboundingboxes)-i-1]
